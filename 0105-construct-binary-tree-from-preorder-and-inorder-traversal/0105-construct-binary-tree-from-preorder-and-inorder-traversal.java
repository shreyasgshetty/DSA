...
class Solution {
    ...
    public TreeNode build(int[] preorder, int preStart, int preEnd, int[] inorder, int inStart, int inEnd, HashMap<Integer,Integer> map){
        ...
        root.left = build(preorder,preStart+1,preStart+leftnum,inorder,inStart,inRoot-1,map);
        root.right = build(preorder,preStart+leftnum+1,preEnd,inorder,inRoot+1,inEnd,map);
        return root;
    }
}